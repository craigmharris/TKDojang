...
final class FlashcardServiceIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        // Load JSON terminology data for realistic testing
        try await loadTerminologyData()
    }
    ...
    private func loadTerminologyData() async throws {
        // Load from production JSON files
        guard let url = Bundle.main.url(forResource: "terminology", withExtension: "json"),
              let data = try? Data(contentsOf: url),
              let entries = try? JSONDecoder().decode([TerminologyEntry].self, from: data) else {
            XCTFail("Failed to load terminology JSON data")
            return
        }
    ...
    func testConfigurationToTermSelectionFlow() throws {
        ...
        let belt8thKeup = BeltLevel.keup8  // White belt - fewer terms
        let belt1stKeup = BeltLevel.keup1  // Red belt - more terms
    ...
    func testAnswerRecordingToProgressTrackingFlow() throws {
        ...
        let progressDescriptor = FetchDescriptor<UserTerminologyProgress>(
            predicate: #Predicate { progress in
                progress.terminologyEntry.id == testTerm.id
            }
        )

        let progressRecords = try testContext.fetch(progressDescriptor)
        ...
        if let progress = progressRecords.first {
            // PROPERTY: Attempt count increments
            XCTAssertGreaterThan(
            ...
            // PROPERTY: Last seen date updated
            XCTAssertNotNil(
                progress.lastSeenAt,
                "Last seen date should be recorded"
            )
        }
    }
    ...
    func testSessionCompletionToStatsRecordingFlow() throws {
        ...
        let sessionType: SessionType = .flashcards
        let itemsStudied = 23
    ...
    func testLearningModeIntegrationFlow() throws {
        ...
        let profile = try createTestProfile(
            name: "Mode Test",
            beltLevel: .keup2,  // Advanced belt to test mastery mode
            learningMode: .progression
        )
        ...
        XCTAssertTrue(
            progressionTerms.allSatisfy { $0.beltLevel.sortOrder == BeltLevel.keup2.sortOrder },
            "All progression terms should be from current belt"
        )

        // ACT: Switch to mastery mode
        profile.learningMode = .mastery
        try testContext.save()
        ...
        XCTAssertTrue(
            masteryTerms.contains { $0.beltLevel.sortOrder == BeltLevel.keup2.sortOrder },
    ...
    func testConfigurationValidationFlow() throws {
        ...
        let beginnerProfile = try createTestProfile(
            name: "Beginner",
            beltLevel: .keup10  // Lowest belt - minimal terms
        )
    ...
    func testCompleteFlashcardWorkflow() throws {
        ...
        let initialFlashcards = profile.totalFlashcardsSeen
        let initialSessions = profile.totalStudySessions
        ...
        XCTAssertGreaterThan(
            profileAfter.totalStudySessions,
            initialSessions,
